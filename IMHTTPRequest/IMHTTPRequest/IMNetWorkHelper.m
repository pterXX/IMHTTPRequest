...
@implementation IMNetWorkHelper
...
+ (void)networkStatusWithBlock:(IMNetworkStatus)networkStatus {
    [[AFNetworkReachabilityManager sharedManager] setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
        switch (status) {
            ...
            networkStatus ? networkStatus(IMNetworkStatusUnknown) : nil;
            if (_isOpenLog) NSLog(@"未知网络");
            break;
            case AFNetworkReachabilityStatusNotReachable:
            networkStatus ? networkStatus(IMNetworkStatusNotReachable) : nil;
            if (_isOpenLog) NSLog(@"无网络");
            break;
            case AFNetworkReachabilityStatusReachableViaWWAN:
            networkStatus ? networkStatus(IMNetworkStatusReachableViaWWAN) : nil;
            if (_isOpenLog) NSLog(@"手机自带网络");
            break;
            case AFNetworkReachabilityStatusReachableViaWiFi:
            networkStatus ? networkStatus(IMNetworkStatusReachableViaWiFi) : nil;
...
+ (void)showRequestNetworkingSetting{
    ...
    NSString *title = [NSString stringWithFormat:@"允许\"%@\"使用数据?", app_Name];
...
+ (NSURLSessionTask *)GET:(NSString *)URL
    ...
    {
        //读取缓存
        responseCache!=nil ? responseCache([IMNetworkCache httpCacheForURL:URL parameters:parameters]) : nil;
    }
...
//            NSLog(@"responseObject = %@",responseObject);
        ...
        //对数据进行异步缓存
        responseCache!=nil ? [IMNetworkCache setHttpCache:responseObject URL:URL parameters:parameters] : nil;
...
+ (void)setRequestSerializer:(IMRequestSerializer)requestSerializer {
    _sessionManager.requestSerializer = requestSerializer==IMRequestSerializerHTTP ? [AFHTTPRequestSerializer serializer] : [AFJSONRequestSerializer serializer];
...
+ (void)setResponseSerializer:(IMResponseSerializer)responseSerializer {
    _sessionManager.responseSerializer = responseSerializer==IMResponseSerializerHTTP ? [AFHTTPResponseSerializer serializer] : [AFJSONResponseSerializer serializer];
}
